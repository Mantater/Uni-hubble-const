...
def load_ceph_data(file_path):
    ...
    """
    ceph_name = np.loadtxt(file_path, 
                           unpack=True, 
                           usecols=(0), 
                           skiprows=1, 
                           dtype=str
    ...
    log10_P_ceph, app_mag_ceph = np.loadtxt(file_path, 
                                            unpack=True, 
                                            usecols=(1, 2), 
                                            skiprows=1, 
                                            dtype=float
...
# Plot IQR diagram to show outliers
ax2.set_title("IQR diagram for d_Mpc")
# Calculate Q1 (25th percentile) and Q3 (75th percentile)
Q1 = np.percentile(d_pc_ceph, 25)
Q3 = np.percentile(d_pc_ceph, 75)
...
# Create a boxplot
ax3.boxplot(d_pc_ceph, vert=False)
...
for outlier in outliers:
    while outlier in d_pc_ceph:
        del d_pc_ceph_err[d_pc_ceph.index(outlier)]
        d_pc_ceph.remove(outlier)

print("Outliers:", outliers[0])

# Add labels and title
ax3.set_title('IQR Diagram to Show Outliers')
ax3.set_xlabel('Distance (Mpc)')
ax3.legend(['Data', 'Q1', 'Q3', 'IQR', 'Lower Bound', 'Upper Bound', 'Outliers'])
#plt.show()
...
def load_galaxy_data(file_path):
    ...
    d_Mpc_gal, d_Mpc_gal_err = np.loadtxt(file_path, 
                                          unpack=True, 
                                          usecols=(2, 3), 
                                          skiprows=2, 
                                          dtype=float
